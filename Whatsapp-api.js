	// Send 
async function send_message(text, times=1, selfdestruct=false){
    ...
    if(selfdestruct){
        await sleep(1000).then(function(){
            ...
            setInterval(function(){if(seen(sent[0])){delete_message(sent);}},5000);
        });
    }
}
...
async function delete_message(messages, only_for_me=false){
    ...
    while(canvas == null){
            await sleep(Math.random()*400+100).then(function(){
            canvas = document.querySelector("div[class='_1_keJ']");
        });
